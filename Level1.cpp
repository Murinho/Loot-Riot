...
bool Level1::init()
{
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(Level1::timerMethod), 0.25f);


    gameAudioProfile = cocos2d::AudioEngine::INVALID_AUDIO_ID;

    if (gameAudioProfile == cocos2d::AudioEngine::INVALID_AUDIO_ID) {
        gameAudioProfile = cocos2d::AudioEngine::play2d("music/desert_music.mp3", true, 1.0f);
    }
    ...
    auto grid = Sprite::create("grid_mod.png");
...
void Level1::timerMethod(float dt) {
    tempo += dt;
    if (newheart) {
        heartGen();
    }
...
void Level1::spriteGen() {
    ...
    if (startp == 1) { //start from north
        auto action = MoveBy::create(3, Point(0, -400));
        ...
        auto action = MoveBy::create(3, Point(400, 0));
        int indx = randomizer(1, 13);
...
void Level1::boxBreaks(Point boxcoords) {
